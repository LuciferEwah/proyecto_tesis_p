...
class ScreenJuicioMedidasCautelares(ft.UserControl):
    def __init__(self, db, page, data, id_juicio, update_tabla_juicio_callback=None, mostrar_mensaje_func=None):
        super().__init__()
        self.database = db
        self.page = page
    ...
    def build(self):
        ...
        self.button_update = ft.OutlinedButton(
            "Guardar Medida Cautelar del Juicio" if self.data else "Guardar Nueva Medida Cautelar del Juicio",
            icon=ft.icons.SAVE,
            icon_color="green400",
        ...
        self.button_delete = ft.OutlinedButton(
            "Eliminar Medida Cautelar del Juicio",
        ...
        self.medida_cautelar_section = FormCardWidget(
            title="Detalles de la Medida Cautelar del Juicio",
    ...
    async def actualizar_tipos_de_medidas(self, id_juicio):
        # Obtener relaciones existentes de la base de datos
        relaciones_existentes_raw = await self.database.obtener_tipos_medidas_provisorias_por_juicio(id_juicio)
    ...
    async def inicializar_opciones_menu(self):
        opciones_filtradas, todas_las_opciones_raw = await self.actualizar_tipos_de_medidas(self.id_juicio)
    ...
    async def some_confirm_action(self):
        try:
            ...
            medida_cautelar = MedidaCautelar(
                id_evento=self.id_juicio,  # Ajuste para id_juicio
            ...
            if id_medida_cautelar is None or id_medida_cautelar == '':
                await self.database.add_medida_provisoria_juicio(self.id_juicio, medida_cautelar)
                mensaje = "Nueva medida cautelar de juicio agregada correctamente"
                await self.mostrar_mensaje(self.page, mensaje, severidad='exito')
                self.page.views.pop()
                top_view = self.page.views[-1]
                await self.page.go_async(top_view.route)
            else:
                await self.database.update_medida_provisoria_juicio(id_medida_cautelar, medida_cautelar)
                mensaje = "Medida cautelar de juicio actualizada correctamente"
                await self.mostrar_mensaje(self.page, mensaje, severidad='exito')
    ...
    async def some_delete_action(self, event=None):
        try:
            id_medida_cautelar = self._get_data_value(0)
            await self.database.delete_medida_provisoria_juicio(id_medida_cautelar)
            mensaje = "Medida cautelar de juicio eliminada correctamente"
